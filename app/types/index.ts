...
export interface FutsalMatch {
  ...
  venue: string
  referee?: string
  status: "Scheduled" | "Active" | "Lineup Set" | "Live" | "Finished"
  homeScore: number | null
  awayScore: number | null
  teamSize: 5 | 6 | 7
  matchDuration: number // Total playing duration (sum of all non-break periods)
  // Removed firstHalfDuration and secondHalfDuration as they are replaced by periods
  periods: MatchPeriod[] // Array of all periods in the match
  periodState?: FutsalMatchPeriodState // Current state of periods
  maxSubstitutions: number
  homeLineup?: FutsalTeamLineup
  awayLineup?: FutsalTeamLineup
  startTime?: Date // Overall match start time
  currentMinute?: number // Total elapsed playing minutes (sum of active periods)
  goals?: Goal[]
  events?: FutsalMatchEvent[]
  // New: Track period-specific timing
  periodTimings?: {
    [periodId: string]: {
      startTime?: Date
      endTime?: Date
...
export interface FutsalMatchState {
  ...
  // New: Current half duration being used
  currentHalfDuration?: number
  // New: Track which half we're in for timing calculations
  currentHalf: 1 | 2
}
export interface TeamLineup {
  formation: string
...
// New interface for multiple periods support
export interface MatchPeriod {
  id: string
  name: string
  duration: number // in minutes
  orderIndex: number
  breakPeriod?: boolean // true for break periods like half-time
  breakDuration?: number // duration of break after this period
}
